...
var notcieConfig = {
  ...
  searchEvent: function () {
    ...
    if (this.data.searchText) {
      if (_this.data.tabNav == "all") {
        Utils.Request(`${API.CleanList}`, { title: _this.data.searchText,userid: wx.getStorageSync('userid') }).then((res) => {
          res.statusCode === 200 && _this.setData({ cleanData: _this.dataFormat(res.data.rows) });
        });
      } else {
        Utils.Request(`${API.CleanList}`, { title: _this.data.searchText,cart: _this.data.tabNav, userid: wx.getStorageSync('userid') }).then((res) => {
          res.statusCode === 200 && _this.setData({ cleanData: _this.dataFormat(res.data.rows) });
        });
      }
      ...
      if (_this.data.tabNav == "all") {
        Utils.Request(`${API.CleanList}`, { userid: wx.getStorageSync('userid') }).then((res) => {
          res.statusCode === 200 && _this.setData({ cleanData: _this.dataFormat(res.data.rows) });
        });
      } else {
        Utils.Request(`${API.CleanList}`, { cart: _this.data.tabNav, userid: wx.getStorageSync('userid') }).then((res) => {
          res.statusCode === 200 && _this.setData({ cleanData: _this.dataFormat(res.data.rows) });
        });
      }
    }
  ...
  onLoad: function () {
    let _this = this;
    this.getData.call(this, { userid: wx.getStorageSync('userid') });
    Utils.Request(`${API.CleanCart}`, { userid: wx.getStorageSync('userid') }).then((res) => {
      res.statusCode === 200 && _this.setData({ navData: res.data });
    });
  },
  getData:function(cart={}){
    let _this = this;
    Utils.Request(`${API.CleanList}`,cart).then((res) => {
      res.statusCode === 200 && _this.setData({ cleanData: _this.dataFormat(res.data.rows) });
