...
public class UserController extends AdminBaseController {
	...
	@RequiresUser
	public void getuser() {
		String uid = ShiroKit.getUser().getId().toString();
        String deptId = ShiroKit.getUser().getDeptId().toString();
        boolean ynsuper = CommonUtil.ynsuper();

        Map map = new HashMap();
		map.put("uid",uid);
		map.put("did",deptId);
		map.put("isadmin",ynsuper);
		renderJson(map);
	}
	...
	public void findname() {
        ...
        for(String id:ids.split(",")){
            User byId = Blade.create(User.class).findById(id);
            if(byId!=null){
                Map m = new HashMap();
                m.put("name",byId.getName());
                m.put("id",id);
                list.add(m);
            }
        }
	...
	public void userchoose() {
        String who = getPara("who","leader");

        setAttr("who",who);
        setAttr("code", CODE);
        if("do".equals(who)) {//执行
			setAttr("fid", getPara("fid", "1"));
			render(BASE_PATH + "userchoose2.html");
		}else if("jiaqian".equals(who)){//加签
			setAttr("fid", getPara("fid", "1"));
			render(BASE_PATH + "userchoose3.html");
		}else if("casheradd".equals(who)){//加签
			setAttr("cid", getPara("cid", "1"));
			render(BASE_PATH + "userchoose4.html");
		}else {
			render(BASE_PATH + "userchoose.html");
		}
